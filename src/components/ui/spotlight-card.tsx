...
interface GlowCardProps {
  ...
  /** When true, ignores size prop and uses width/height or className for layout. */
  customSize?: boolean;
}
...
const SPOTLIGHT_STYLE_ID = "glow-card-spotlight-styles";
...
const GlowCard = ({
  ...
  const getInlineStyles = (): CSSProperties => {
    // Use --glow-border-width (not --border) to avoid clashing with theme --border color.
    const baseStyles: CSSProperties & Record<string, string | number> = {
      "--base": base,
      "--spread": spread,
      "--radius": customSize ? "16" : "14",
      "--glow-border-width": customSize ? "1" : "3",
      "--backdrop": customSize ? "transparent" : "hsl(0 0% 60% / 0.12)",
      "--backup-border": customSize ? "var(--color-border)" : "var(--backdrop)",
      "--size": "200",
      "--outer": "1",
      "--border-size": "calc(var(--glow-border-width, 2) * 1px)",
  ...
  const demoLayoutClasses = customSize
    ? ""
    : "aspect-[3/4] grid grid-rows-[1fr_auto] shadow-[0_1rem_2rem_-1rem_black] gap-4 p-4 backdrop-blur-[5px]";
  ...
  return (
    <div
      ...
      className={[
        getSizeClasses(),
        demoLayoutClasses,
        customSize ? "relative" : "relative rounded-2xl",
        className,
      ]
        .filter(Boolean)
        .join(" ")}
